...
namespace RCPJ.DAL.Ruc
{
    public class PSC_PROTOCOLO : DBInteractionBaseORACLE
    {
        ...
        public static int IsExistProtocoloRegin(string pProtocolo, string pNire)
        {
            using (OracleConnection _conn = new OracleConnection(General.ConnectionString()))
            {
                ...
                using (OracleCommand cmdToExecute = new OracleCommand())
                {
                    ...
                    Sql.AppendLine(" From   psc_ident_protocolo ip ");
                    Sql.AppendLine(" Where  1 = 1 ");
                    Sql.AppendLine(" And    ip.pip_pro_protocolo = '" + pProtocolo + "'");
                    Sql.AppendLine(" Or     ip.pip_protocolo_junta = '" + pProtocolo + "'");
                    ...
                    return int.Parse(pCount.ToString());
                }
            }
        }
        public static int IsExistDBE(string pDBE)
        {
            using (OracleConnection _conn = new OracleConnection(General.ConnectionString()))
            {
                _conn.Open();
